...
export async function transcribeAudio(apiKey: string, b64: string, mime: string) {
  const bin = atob(b64)
  ...
  fd.append('file', new Blob([bytes], { type: mime.split(';')[0] }), `audio.${ext}`)
...
export async function callOpenAI(apiKey: string, model: string, sys: string, msgs: any[]) {
...
export async function callGemini(apiKey: string, model: string, sys: string, msgs: any[]) {
  const norm = normalizeMessages(msgs).map((m) => {
    const parts: any[] = []
...
export async function callClaude(apiKey: string, model: string, sys: string, msgs: any[]) {
  const norm = normalizeMessages(msgs).map((m) => {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: alternating.length > 0 ? alternating : [{ role: 'user', content: 'Hello' }],
    }),
  })
...
export async function generateChatResponse(
  ...
  if (provider === 'openai') return callOpenAI(apiKey, 'gpt-4o-mini', systemPrompt || '', messages)
  if (provider === 'gemini')
    return callGemini(apiKey, 'gemini-1.5-flash', systemPrompt || '', messages)
  if (provider === 'claude')
    return callClaude(apiKey, 'claude-3-haiku-20240307', systemPrompt || '', messages)
  throw new Error(`Provider ${provider} not supported for chat generation`)
}

export async function generateEmbedding(text: string, apiKey: string, provider: string = 'openai') {
  if (provider === 'gemini') {
    ...
    if (!response.ok) throw new Error(`Gemini Embedding API error: ${await response.text()}`)
  ...
  if (!response.ok) throw new Error(`OpenAI Embedding API error: ${await response.text()}`)
