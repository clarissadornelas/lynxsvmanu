...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { entrevista_id, vaga_id, candidato_id } = body

    if (!entrevista_id || !vaga_id || !candidato_id) {
...
Generate 3-5 categories with 1-3 questions each. All content must be in Brazilian Portuguese.` +
    ...
    await supabase
      .from('entrevistas')
      .update({ roteiro: JSON.stringify(roteiro), status: 'roteiro_pronto' })
      .eq('id', entrevista_id)
    ...
    console.error('copilot-generate-prep error:', error)
