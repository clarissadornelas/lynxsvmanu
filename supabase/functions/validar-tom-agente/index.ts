...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(
        JSON.stringify({ permitido: false, motivo: 'Não autorizado' }),
    ...
    if (keyErr || !keyRow || !keyRow.api_key_encrypted) {
      return new Response(
        JSON.stringify({ permitido: false, motivo: 'Nenhuma chave de provedor de IA configurada para esta empresa.' }),
        { status: 200, headers: { 'Content-Type': 'application/json', ...corsHeaders } }
      );
    }
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ permitido: false, motivo: 'Falha ao descriptografar a chave de IA. Contate o suporte.' }),
        { status: 200, headers: { 'Content-Type': 'application/json', ...corsHeaders } }
      );
    }

    const validationContext = contexto === 'criterios' ? 'criterios' : 'tom';
    const systemPrompt = validationContext === 'criterios'
      ? CRITERIOS_SYSTEM_PROMPT
      : TOM_SYSTEM_PROMPT;
    ...
    return new Response(
      JSON.stringify({ permitido: false, motivo: 'Erro interno do servidor.' }),
      { status: 200, headers: { 'Content-Type': 'application/json', ...corsHeaders } }
    );
  }
});
