...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await supabase.from('agent_knowledge_embeddings').delete().eq('agent_id', agentId)
    ...
    await supabase.from('ai_agents').update({ knowledge_base_status: 'ready' }).eq('id', agentId)
    ...
    if (supabase && agentId) {
      await supabase.from('ai_agents').update({ knowledge_base_status: 'error' }).eq('id', agentId)
    }
